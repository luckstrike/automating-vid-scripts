...
async function summarizeParsedInfo(openai: OpenAI, HTMLResult: string): Promise<string | null> {
...
async function runParser(userAgent: string, url: string): Promise<string | null> {
    ...
    if (await checkIfAllowed(userAgent, url)) {
        ...
        console.error("Uh oh, seems like we're not allowed to scrape this website :(")
    }
